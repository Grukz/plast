...
def argparser(parser, modules={}):
    ...
    parser.add_argument(
        ...
        help="select the callback(s) that will handle the resulting data <default=*>")

    parser.add_argument(
        "--format", choices=["json"], default="json",
        help="output format for detection(s) <default=\"json\">")
    ...
    parser.add_argument(
        ...
        help="output format for detection(s), see hashlib API reference for supported algorithm(s) <default=md5,sha1,sha256>")

    parser.add_argument(
        "--logging", choices=["debug", "info", "warning", "error", "critical", "suppress"], default="info",
        help="override the default console logging level <default=info>")
    ...
    parser.add_argument(
        ...
        help="select the postprocessor(s) that will handle the resulting data <default=*>")

    parser.add_argument(
        "--processes", type=int, choices=range(1, 100), default=(multiprocessing.cpu_count() or 4), metavar="NUMBER",
        help="override the number of concurrent processe(s) <default={}>".format(multiprocessing.cpu_count() or 4))
